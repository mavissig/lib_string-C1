...
2. [Information](#information) \
   2.1. [Типы](#stringh-типы) \
   2.2. [Макросы](#stringh-макросы) \
   2.3. [Функции](#stringh-функции) \
   2.4. [sprintf and sscanf](#sprintf-and-sscanf) \
   2.5. [sprintf and sscanf Спецификаторы](#sprintf-and-sscanf-спецификаторы) \
   2.6. [sprintf Флаги](#sprintf-флаги) \
   2.7. [sprintf and sscanf Ширина](#sprintf-and-sscanf-ширина) \
   2.8. [sprintf Точность](#sprintf-точность) \
   2.9. [sprintf and sscanf Длина](#sprintf-and-sscanf-длина) \
   2.10. [Специальные функции обработки строк](#специальные-функции-обработки-строк-вдохновленные-классом-string-в-языке-c)
3. [Реализация функций библиотеки string.h](#реализация-функций-библиотеки-stringh)  
4. [Частичная реализация функции sprintf](#частичная-реализация-функции-sprintf)  
5. [Реализация некоторых модификаторов формата функции sprintf](#реализация-некоторых-модификаторов-формата-функции-sprintf)  
6. [Реализация функции sscanf](#реализация-функции-sscanf)  
7. [Реализация специальных функций обработки строк](#реализация-специальных-функций-обработки-строк)


## Preamble

![string+](misc/rus/images/stringplus.png)
...
В данном проекте я разработал собственную реализацию библиотеки string.h на языке программирования Си с некоторыми дополнениями 
(с собственной реализацией функций sprintf и sscanf). Библиотека string.h является основной библиотекой языка Си по обработке строк. 
В рамках этого проекта предполагалась отработка задач на работу со строковыми данными и закрепление структурного подхода.
...
- Код библиотеки, включая заголовочные файлы, мейкфайлы и сама библиотека находятся в папке src
...
- Оформлено решение как статическая библиотека (с заголовочным файлом s_string.h)
- Библиотека разработана в соответствии с принципами структурного программирования, постарался исключить дублирование в коде
- Выполнено полное покрытие unit-тестами функций библиотеки c помощью библиотеки Check
- Unit-тесты проверяют результаты работы моей реализации путём сравнения ее с реализацией стандартной библиотеки string.h
- Unit-тесты покрывают не менее 80% каждой функции
- Предусмотрен Makefile для сборки библиотеки и тестов (с целями all, clean, test, s_string.a, gcov_report)
- В цели gcov_report формируется отчёт gcov в виде html страницы.
- Для исключения конфликтов перед каждой функцией использовался префикс `s_`

## Частичная реализация функции sprintf

Была реализована функция sprintf из библиотеки stdio.h:
- Функция была размещена в библиотеке s_string.h
- На реализацию функции накладывались все требования, изложенные в [первой части](#реализация-функций-библиотеки-stringh).
...
Были реализованы некоторые модификаторы формата функции sprintf из библиотеки stdio.h:
- Функция размещена в библиотеке s_string.h.
- На реализацию функции накладывались все требования, изложенные в [первой части](#реализация-функций-библиотеки-stringh).
...
Была реализована функция sscanf из библиотеки stdio.h:
- Функция размещена в библиотеке s_string.h.
- На реализацию функции накладывались все требования, изложенные в [первой части](#реализация-функций-библиотеки-stringh).
...
Были реализованы некоторые функции обработки строк из класса String (описанные [здесь](#специальные-функции-обработки-строк-вдохновленные-классом-string-в-языке-c):
- Функции размещены в библиотеке s_string.h.
- На реализацию функций накладываются все требования, изложенные в [первой части](#реализация-функций-библиотеки-stringh)
